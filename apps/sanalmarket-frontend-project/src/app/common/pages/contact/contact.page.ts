...
export class ContactPage extends SubscriptionAbstract implements OnInit {
  ...
  fetchPageContent(): void {
    this._faqService
      ...
      .subscribe((data) => {
        ...
        this.answer = data
          .filter((item) => item.question === 'Destek')[0]
          ?.children.filter((item) => item.question === 'İletişim')[0];
      });
  }
